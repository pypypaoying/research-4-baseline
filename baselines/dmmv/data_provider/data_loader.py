...
class Dataset_ETT_minute(Dataset):
    ...
    def __read_data__(self):
        ...
        border1s = [
            0,
            12 * 30 * 24 * 4 - self.history_len,
            12 * 30 * 24 * 4 + 4 * 30 * 24 * 4 - self.history_len,
        ]
        border2s = [
            12 * 30 * 24 * 4,
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.cols:
            ...
            cols.remove("date")
        df_raw = df_raw[["date"] + cols + [self.target]]
        border1 = len(df_raw) - self.history_len
        border2 = len(df_raw)
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        if self.inverse:
            seq_y = self.data_x[r_begin : r_begin + self.overlap_len]
        else:
            seq_y = self.data_y[r_begin : r_begin + self.overlap_len]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
    ...
    def __len__(self):
        return len(self.data_x) - self.history_len + 1

    def inverse_transform(self, data):
        return self.scaler.inverse_transform(data)


class Dataset_Custom_Split_Variable(Dataset):
    def __init__(
        self,
        root_path,
        flag="train",
    ...
    def __len__(self):
        return (len(self.data_x) - self.history_len - self.pred_len + 1) * self.channels
